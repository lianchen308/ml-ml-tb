% function [min_auc, min_acc] = mabeval(learners, weights, X_train, y_train, X_test, y_test) 
function [min_auc, min_acc] = mabeval(learners, weights, X_train, y_train, X_test, y_test) 
    ...
    min_auc = min([y_train_auc y_test_auc y_all_auc]);
    min_acc = min([y_train_acc y_test_acc y_all_acc]);
